...
def load_and_train():
    ...
    try:
        ...
        st.error("Could not load dataset from URL.")
        st.stop()
    ...
    return trained, results, best_name, best_model, feat_imp, df, y, X.columns.tolist()


with st.spinner("Training 3 models on 284,000+ transactions... (about 60 seconds)"):
    trained, results, best_name, best_model, feat_imp, df, y, feature_cols = load_and_train()

fraud = df[df["Class"] == 1]
legit = df[df["Class"] == 0]

st.markdown('<p class="main-title">🔍 Credit Card Fraud Detection System</p>', unsafe_allow_html=True)
st.markdown('<p class="sub-title">Machine Learning · Random Forest + XGBoost + SMOTE · 284,807 Real Transactions</p>', unsafe_allow_html=True)
st.markdown("")
...
with tab1:
    ...
    if predict_btn:
        ...
        for col, val in {"Amount_Scaled": amount_scaled, "Time_Scaled": time_scaled,
                          "V1": v1, "V2": v2, "V3": v3, "V4": v4, "V5": v5,
                          "V14": v14, "V17": v17, "V12": v12,
                          "V10": v10, "V11": v11}.items():
...
st.markdown(
    ...
    'Built by <b>Dilawar Mahar</b> · Random Forest + XGBoost + SMOTE · '
